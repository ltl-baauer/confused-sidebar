...
class Frame {
  ...
  async render(now) {
    ...
    const tree = grouped ? treeRows(displayEntries, workspaces, tabs) : new Map();
    ...
    for (const entry of entries) {
      ...
      const indent = '';
      const corner = tree.get(entry.pane)?.prefix ?? '';
    ...
    await this.groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines, tabs);
  ...
  async groupJobs(entries, displayEntries, viewMode, grouped, wsAgents, workspaces, keys, now, deadlines, tabs) {
    ...
    const fingerprint =
      ...
      `${[...keys.orphanRepo]
        ...
        .sort()
        .join('+')}:` +
      displayEntries.map((e) => `${e.workspace}/${e.tab}/${e.pane}/${workspaces.get(e.workspace)}/${tabs.get(e.tab)}`).join(',');
